...
from shodic.ingestion.storage import save_raw_record
from shodic.ingestion.nafdac_parser import parse_nafdac, products_to_chunks
from shodic.ingestion.nafdac_scraper import parse_detail_html, parse_listing_html
from shodic.ingestion.source_status import source_status_rows
from shodic.models import DrugChunk, RawSourceData, SOURCE_CHOICES, SourceFileUpload
...
class IngestionPipelineTests(TestCase):
    ...
    @patch("shodic.management.commands.ingest_drugs.upsert_drug_chunks", return_value=1)
    ...
    @patch("shodic.qdrant_service.delete_points")
...
class QdrantHybridTests(TestCase):
    ...
    @patch("shodic.qdrant_service._get_client")
    def test_upsert_uses_dense_and_bm25_named_vectors(self, get_client):
        ...
        self.assertEqual(fake_client.collection_name, "shodic")
    ...
    def test_retrieve_uses_dense_and_bm25_prefetches(self, get_client):
        from shodic.qdrant_service import retrieve_context
...
class IngestionAdminTests(TestCase):
    ...
    def test_admin_index_shows_shodic_admin(self):
        response = self.client.get("/admin/")

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "SHODIC Administration")
        self.assertContains(response, 'class="app-shodic')
        self.assertContains(response, "Upload source files")

    @override_settings(ROOT_URLCONF="config.urls")
    def test_admin_ingestion_page_loads_for_superuser(self):
        response = self.client.get("/admin/shodic/ingestion/")
    ...
    def test_admin_source_file_upload_add_queues_processing(self, queue_task):
        with tempfile.TemporaryDirectory() as tmp:
            with override_settings(MEDIA_ROOT=tmp):
                response = self.client.post(
                    "/admin/shodic/sourcefileupload/add/",
                    {
                        "source": "emdex",
    ...
    def test_admin_raw_source_changelist_loads(self):
        response = self.client.get("/admin/shodic/rawsourcedata/")
