...
from shodic.ingestion.nafdac_scraper import NAFDAC_CATEGORIES
from shodic.ingestion.source_status import source_status_rows
from shodic.ingestion.update_checker import check_all_sources

admin.site.site_header = 'SHODIC Administration'
admin.site.site_title = 'SHODIC Admin'
...
def can_run_ingestion(user) -> bool:
    return bool(user.is_active and (user.is_superuser or (user.is_staff and user.has_perm("shodic.can_run_ingestion"))))
...
def ingestion_admin_view(request: HttpRequest):
    ...
    if request.method == "POST":
        ...
        return redirect(reverse("admin:shodic_ingestion"))
    ...
    return render(request, "admin/shodic/ingestion.html", context)
