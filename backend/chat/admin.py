...
from chat.ingestion.nafdac_scraper import NAFDAC_CATEGORIES
from chat.ingestion.source_status import source_status_rows
from chat.ingestion.update_checker import check_all_sources
...
def can_run_ingestion(user) -> bool:
    return bool(user.is_active and (user.is_superuser or (user.is_staff and user.has_perm("chat.can_run_ingestion"))))
...
def ingestion_admin_view(request: HttpRequest):
    ...
    if request.method == "POST":
        ...
        return redirect(reverse("admin:chat_ingestion"))
    ...
    return render(request, "admin/chat/ingestion.html", context)
...
def _get_urls():
    custom = [
        path("chat/ingestion/", admin.site.admin_view(ingestion_admin_view), name="chat_ingestion"),
    ]
