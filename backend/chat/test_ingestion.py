...
from chat.ingestion.storage import save_raw_record
from chat.ingestion.nafdac_parser import parse_nafdac, products_to_chunks
from chat.ingestion.nafdac_scraper import parse_detail_html, parse_listing_html
from chat.ingestion.source_status import source_status_rows
from chat.models import DrugChunk, RawSourceData, SOURCE_CHOICES, SourceFileUpload
...
class IngestionPipelineTests(TestCase):
    ...
    @patch("chat.management.commands.ingest_drugs.upsert_drug_chunks", return_value=1)
    ...
    @patch("chat.qdrant_service.delete_points")
...
class QdrantHybridTests(TestCase):
    ...
    @patch("chat.qdrant_service._get_client")
    def test_upsert_uses_dense_and_bm25_named_vectors(self, get_client):
        from chat.qdrant_service import upsert_drug_chunks
...
class IngestionAdminTests(TestCase):
    ...
    def test_admin_source_file_upload_add_queues_processing(self, queue_task):
        with tempfile.TemporaryDirectory() as tmp:
            with override_settings(MEDIA_ROOT=tmp):
                response = self.client.post(
                    "/admin/chat/sourcefileupload/add/",
                    {
                        "source": "emdex",
    ...
    def test_admin_changelist_uses_compact_header(self):
        response = self.client.get("/admin/chat/rawsourcedata/")
    ...
    def test_admin_ingestion_requires_permission(self):
        ...
        response = self.client.get("/admin/chat/ingestion/")
